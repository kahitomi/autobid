...
class Seq2SeqModel(object):
	...
	def __init__(self, input_size, output_size, bucket, size, num_layers, max_gradient_norm, batch_size, learning_rate, learning_rate_decay_factor, use_lstm=False, forward_only=False):
		...
		# single_cell = tf.nn.rnn_cell.GRUCell(size, activation=tf.nn.relu6)
		if use_lstm:
		...
		for i in xrange(bucket[1] + 1):
			self.decoder_inputs.append(tf.placeholder(tf.float32, shape=[None, output_size], name="decoder{0}".format(i)))
	...
	def step(self, session, encoder_inputs, decoder_inputs, forward_only):
		...
		input_feed[last_target] = np.zeros([self.batch_size, self.output_size])
...
def model_test():
	...
	with tf.Session() as sess:
		...
		model = Seq2SeqModel(input_size=10, output_size=3, bucket=bucket, size=100, num_layers=2, max_gradient_norm=5.0, batch_size=20, learning_rate=0.1, learning_rate_decay_factor=0.9)
		...
		for _ in range(data_set_size):
			...
			item_output = [0.0 for _ in range(model.output_size)]
			for _ in range(1):
				item_output[random.randint(0, model.output_size-1)] = 1.0
			data_set.append([item_input, item_output])
		...
		for _ in xrange(2001):  # Train the fake model.
			...
			for x in range(model.batch_size):
				seed = random.randint(0, data_set_size-1)
				for i in range(bucket[0]):
					encoder_inputs[i].append(data_set[seed][0][i])
				for i in range(bucket[1]):
					decoder_inputs[i].append(data_set[seed][1])

			decoder_inputs = decoder_inputs[:-1]
			decoder_inputs = [[[0.0 for x in range(model.output_size)] for x in range(model.batch_size)]] + decoder_inputs
